...
def calculate_diversity_loss(attn_pos, attn_neg):
    # Average attention weights across heads
    # input: (batch_size, num_heads, num_prompts, seq_length)

    attn_pos_avg = attn_pos.mean(dim=(1,2))  
    attn_neg_avg = attn_neg.mean(dim=(1,2))
          
    # Calculate cosine similarity between positive and negative attention patterns
    similarity = F.cosine_similarity(
            attn_pos_avg.view(-1, attn_pos_avg.size(-1)),
            attn_neg_avg.view(-1, attn_neg_avg.size(-1)),
            dim=1
        )
