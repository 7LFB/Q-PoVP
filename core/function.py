...
def train(model, trainloader, optimizer, writer_dict, epoch, args):
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        loss = losses.mean()
        if args.atten_loss_weight>0:
            loss_atten = calculate_diversity_loss(model.promptG.pos_atten, model.promptG.neg_atten)
            loss += args.atten_loss_weight * loss_atten

        model.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        # check_and_process(model)
